...
Finally, it would also be interesting to see if the NEON tree dataset [@NEONdata] can be used to train a model and make some experiments. This would not allow to experiment on covered trees, but it could still show whether multiple layers of CHM can help delineating individual trees.
